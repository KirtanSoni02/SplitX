...
function LoginWindow(props) {
    ...
    const checkLogin = async (loginData) => {
        const response = await fetch('https://splitx-backend-6iux.onrender.com/expensia/users/login', {
            method: 'POST',
            headers: {
    ...
    const createUser = async (userData) => {
        try{
            const response = await fetch('https://splitx-backend-6iux.onrender.com/expensia/users/create', {
                method: 'POST',
                headers: {
                    'Content-Type' : 'application/json'
                },
