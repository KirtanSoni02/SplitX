...
function GroupDisplay(props) {
        ...
        useEffect( () => {
            if(props.displayGroup)
            {
                ...
                const fetchOwner = async () => {
                    const response = await fetch(`https://splitx-backend-6iux.onrender.com/expensia/users/name?userId=${props.displayGroup.owner}`);
                ...
                const fetchExpenses = async () => {
                    if (props.displayGroup) {
                      ...
                      const expensePromises = expenseIds.map(async (expenseId) => {
                        const response = await fetch(`https://splitx-backend-6iux.onrender.com/expensia/expense/get?expenseId=${expenseId}`);
                        const data = await response.json();
                        if (data.exists) {
                          return data.expense;
                        }
    ...
    const inviteMember = async () => {
        ...
        else
        {
            ...
            const response = await fetch('https://splitx-backend-6iux.onrender.com/expensia/invite', {
                method: 'POST',
                headers: {
